...
- [x] ~~`test:e2e` script — `bunx --bun playwright test`, same reasoning.~~
  ...
  > **Superseded (Node migration + Phase 11).** The `bunx --bun` rationale is obsolete: `src/db/database.ts` now uses `drizzle-orm/postgres-js` (no `import "bun"`), so it runs fine under plain Node, and `.env` is loaded explicitly (`process.loadEnvFile()` in the test env helper, `--env-file-if-exists` for the server). Phase 11 replaced these scripts with orchestrators (`tsx …run-integration.ts` / `…run-e2e.ts`) that launch vitest/playwright via plain `bunx` and the app server via `node`.
