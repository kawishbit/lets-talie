...
- The bare-metal `start` script runs `bun ./dist/server/entry.mjs`, not `node ...` as originally planned — `src/db/database.ts` uses `drizzle-orm/bun-sql` and the `Bun.env` global, so the app only runs under Bun. `package.json`'s `engines` field was corrected from `node` to `bun` to match. (Confirmed with the project owner before changing.)
...
- [x] Add a `start` script to `package.json`: `bun ./dist/server/entry.mjs` (see notes above — `node` doesn't work with this app's Bun-only DB layer) — the bare-metal equivalent of the Docker CMD
